...
class TestSetupLogging:
    ...
    @patch("logging.getLogger")
    def test_setup_logging_with_file(self, mock_get_logger, mock_file_handler, mock_rich_handler):
...
class TestExtractDocumentation:
    ...
    @patch("docs2llm.main.exists", return_value=True)  # Patching the direct import from os.path
    @patch("docs2llm.main.isdir", return_value=True)   # Patching the direct import from os.path
    @patch("builtins.open", new_callable=mock_open)
    def test_extract_from_local_path(
        self, mock_file, mock_isdir, mock_exists, mock_getsize,
        mock_process, mock_find, mock_setup_logging
    ):
    ...
    def test_extract_from_git_repo(
        self, mock_file, mock_getsize, mock_process, mock_find, 
        mock_clone, mock_temp_dir, mock_setup_logging
    ):
    ...
    @patch("os.path.isdir")
    @patch("docs2llm.main.find_documentation_files")
    def test_no_documentation_files(self, mock_find, mock_isdir, mock_exists, mock_setup_logging):
